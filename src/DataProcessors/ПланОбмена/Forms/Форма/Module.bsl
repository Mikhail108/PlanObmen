...
&НаСервере
Функция ВыгрузитьДанныеTXT(ФайлTXT)  
	...
	ФайлTXT = Каталог + ИмяФайла + ".txt";   
	
	ДанныеДляОбмена.Записать(ФайлTXT);
...
КонецФункции
&НаСервере
Функция ВыгрузитьДанныеTXTВZip(ФайлZIP)
	
	Поток = Новый ПотокВПамяти; 
	
	ФайлTXT = ВыгрузитьДанныеTXT(ФайлTXT);
	
	ФайлZIP = Новый ЗаписьZipФайла;;
	ФайлZIP.Открыть(Поток);   
	ФайлZIP.Добавить(ФайлTXT);
	ФайлZIP.Записать();
...
Функция ВыгрузитьФайлыВZIPНаСервере()
	...
	ФайлZIP = ВыгрузитьДанныеTXTВZip(ФайлZIP);
...
Процедура ЗагрузитьДанныеИзZIPНаСервере(Адрес)
	...
	Для Каждого Файл из Файлы Цикл 
		...
		Если Файл.Расширение = ".zip" Тогда
			...
			АрхивСTXT = Новый ЧтениеZipФайла(ИмяФайлаЗип);
			АрхивСTXT.ИзвлечьВсе(ОбменДаннымиЗип);
			АрхивСTXT.Закрыть(); 
			
			ФайлыАрхиваCTxt = НайтиФайлы(ОбменДаннымиЗип, "*.*");
			
			Для Каждого ФайлTXT из ФайлыАрхиваCTxt Цикл  
				
				Если ФайлTXT.Расширение = ".txt"  Тогда 
					
					ИмяФайлаTXT = ФайлTXT.ПолноеИмя;  
					
					Текст = Новый ТекстовыйДокумент;
					Текст.Прочитать(ИмяФайлаTXT);
